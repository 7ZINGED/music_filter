...
for i = 1:length(Fs_array)
    ...
    hfb3_comp = dsp.CICCompensationInterpolator(cic, 'InterpolationFactor', 2, ...
        'PassbandFrequency', Fpass, 'StopbandFrequency', 4*Fs_in -Fpass-18000, ...
        'StopbandAttenuation', 70, 'PassbandRipple', 0.02, 'SampleRate', Fs_in * 4);
...
%% === Stage 3 并行架构系数生成 (Flat半带 与 Comp通用 分离) ===
disp(' ');
disp('==================================================');
disp('正在导出 Stage 3 (Flat与Comp并行独立模块) Verilog 系数...');

W_coeff = 24;
scale = 2^(W_coeff-1) - 1;

% ----------------------------------------------------
% 1. Flat 模式 (半带滤波器) 的专用拆分逻辑
% ----------------------------------------------------
% 特点：必须保持中心点绝对对称，确保存在 Identity 纯延迟相
function [p0, p1] = get_flat_phases(h)
    h = h(:)';
    h = h / (sum(h) / 2); % 增益归一化到 2.0 (0dB)
    
    % 居中补零对齐到 32 抽头，确保中心峰值落在固定位置
    pad_len = 32 - length(h);
    pad_front = floor(pad_len / 2);
    pad_back = ceil(pad_len / 2);
    h_pad = [zeros(1, pad_front), h, zeros(1, pad_back)];
    ...
    % 半带滤波器拆分：
    p0 = h_pad(1:2:end); % Phase 0: 插值支路 (平滑的对称曲线)
    p1 = h_pad(2:2:end); % Phase 1: 恒等支路 (中心包含 7FFFFF，其余接近 0)
end

% ----------------------------------------------------
% 2. Comp 模式 (通用补偿滤波器) 的专用拆分逻辑
% ----------------------------------------------------
% 特点：不在乎对称性，不找中心点，绝对不准在前面补零！
function [p0, p1] = get_comp_phases(h)
    h = h(:)';
    h = h / (sum(h) / 2); % 增益归一化到 2.0 (0dB)
    
    % 暴力末尾补零，强行对齐到 32 抽头 (每相 16 拍)
    h_pad = [h, zeros(1, 32 - length(h))]; 
    
    % 原汁原味的奇偶拆分，完美映射通用 FIR 交织计算
    p0 = h_pad(1:2:end); % Phase 0: 第 1, 3, 5... 个点
    p1 = h_pad(2:2:end); % Phase 1: 第 2, 4, 6... 个点
end

% === 提取两套独立架构的子相 ===
[f48_p0, f48_p1] = get_flat_phases(hw_rom_coeffs.Fs_48k.hfb3_flat);
[f44_p0, f44_p1] = get_flat_phases(hw_rom_coeffs.Fs_44k.hfb3_flat);

[c48_p0, c48_p1] = get_comp_phases(hw_rom_coeffs.Fs_48k.hfb3_comp);
[c44_p0, c44_p1] = get_comp_phases(hw_rom_coeffs.Fs_44k.hfb3_comp);

% ==========================================================
% 定义增强版打印宏 (支持自定义变量名)
% ==========================================================
% 参数说明: arr-系数数组, var_name-系数变量名, cnt_name-计数器变量名
print_lut_ext = @(arr, var_name, cnt_name) arrayfun(@(k) fprintf('                4''d%d: %s = 24''h%06X;\n', k-1, var_name, ...
    mod(round(max(min(arr(k)*scale, scale), -scale-1)), 2^24)), 1:16);

% ==========================================================
% 1. 打印 Stage3_Flat 轨道的系数 (对应 mode_sel == 0)
% ==========================================================
fprintf('\n// === 请复制到 Stage3_MultiMode_FIR.v 的 Flat 轨道 LUT 中 ===\n');
fprintf('    always @(*) begin\n');
fprintf('        case ({sys_fs_sel, phase_sel_flat})\n');
fprintf('            2''b00: case(mac_cnt_flat) // 48k, Phase 0\n'); print_lut_ext(f48_p0, 'coeff_flat', 'mac_cnt_flat'); fprintf('                default: coeff_flat = 24''d0; endcase\n');
fprintf('            2''b01: case(mac_cnt_flat) // 48k, Phase 1\n'); print_lut_ext(f48_p1, 'coeff_flat', 'mac_cnt_flat'); fprintf('                default: coeff_flat = 24''d0; endcase\n');
fprintf('            2''b10: case(mac_cnt_flat) // 44.1k, Phase 0\n'); print_lut_ext(f44_p0, 'coeff_flat', 'mac_cnt_flat'); fprintf('                default: coeff_flat = 24''d0; endcase\n');
fprintf('            2''b11: case(mac_cnt_flat) // 44.1k, Phase 1\n'); print_lut_ext(f44_p1, 'coeff_flat', 'mac_cnt_flat'); fprintf('                default: coeff_flat = 24''d0; endcase\n');
fprintf('            default: coeff_flat = 24''d0;\n');
fprintf('        endcase\n');
fprintf('    end\n');

% ==========================================================
% 2. 打印 Stage3_Comp 轨道的系数 (对应 mode_sel == 1)
% ==========================================================
fprintf('\n// === 请复制到 Stage3_MultiMode_FIR.v 的 Comp 轨道 LUT 中 ===\n');
fprintf('    always @(*) begin\n');
fprintf('        case ({sys_fs_sel, phase_sel_comp})\n');
fprintf('            2''b00: case(mac_cnt_comp) // 48k, Phase 0\n'); print_lut_ext(c48_p0, 'coeff_comp', 'mac_cnt_comp'); fprintf('                default: coeff_comp = 24''d0; endcase\n');
fprintf('            2''b01: case(mac_cnt_comp) // 48k, Phase 1\n'); print_lut_ext(c48_p1, 'coeff_comp', 'mac_cnt_comp'); fprintf('                default: coeff_comp = 24''d0; endcase\n');
fprintf('            2''b10: case(mac_cnt_comp) // 44.1k, Phase 0\n'); print_lut_ext(c44_p0, 'coeff_comp', 'mac_cnt_comp'); fprintf('                default: coeff_comp = 24''d0; endcase\n');
fprintf('            2''b11: case(mac_cnt_comp) // 44.1k, Phase 1\n'); print_lut_ext(c44_p1, 'coeff_comp', 'mac_cnt_comp'); fprintf('                default: coeff_comp = 24''d0; endcase\n');
fprintf('            default: coeff_comp = 24''d0;\n');
fprintf('        endcase\n');
fprintf('    end\n');